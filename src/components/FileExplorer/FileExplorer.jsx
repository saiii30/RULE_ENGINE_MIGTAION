...
export const FileExplorer = observer(({ selectedNode, setPopupOpen, setSelectedNode, setHoverId, hoverId }) => {
  ...
  const handleNodeClick = (node1, level) => {
    ...
    // Show the Rule/Group popup only for level 2 nodes
    if (level === 1) { // Level 2 nodes have an index of 1
      setPopupOpen(true);
      Store.isSidebarVisible1 = false; // Hide other popups if necessary
    } else {
      setPopupOpen(false);
      Store.isSidebarVisible1 = true; // Show the other sidebar for other levels
    ...
    nodes.map((node, index) => (
      <div key={node.id} style={{ marginLeft: level * 10, position: "relative", cursor: "pointer" }}>
        ...
        >
          ...
          {/* Node Name */}
          <span className="node-name" onClick={() => handleNodeClick(node, node.level)}>{node.name}</span>
